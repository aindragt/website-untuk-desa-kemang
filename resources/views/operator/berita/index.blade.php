...
<div class="admin-card">
    <div class="admin-card__header">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap">
            {{-- Filter cari --}}
            <form action="{{ route('operator.berita.index') }}" method="GET" style="display:flex;gap:6px">
                ...
                @if($cari || $kategori)
                    <a href="{{ route('operator.berita.index') }}" class="btn-sm btn-sm--hapus">Reset</a>
                @endif
            </form>
            <a href="{{ route('operator.berita.create') }}" class="btn-sm btn-sm--edit">+ Tulis Berita</a>
        </div>
    </div>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="white-space:nowrap">
                    <a href="{{ route('operator.berita.edit', $b) }}" class="btn-sm btn-sm--edit">✏️ Edit</a>

                    <form action="{{ route('operator.berita.toggle', $b) }}" method="POST" style="display:inline">
                        @csrf @method('PATCH')
            ...
            <tr>
                <td colspan="6" style="text-align:center;padding:3rem;color:var(--teks-muted)">
                    Belum ada berita. <a href="{{ route('operator.berita.create') }}" style="color:var(--emas)">Tulis sekarang →</a>
                </td>
            </tr>
            @endforelse
        </tbody>
